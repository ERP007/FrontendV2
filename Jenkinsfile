        ...
        stage('Health Check') {
            ...
            steps {
                sh '''
                    ...
                    curl -fsS --retry 10 --retry-all-errors --retry-delay 3 --connect-timeout 5 --max-time 10 "https://www.erp007.xyz" >/dev/null
                    curl -fsS --retry 10 --retry-all-errors --retry-delay 3 --connect-timeout 5 --max-time 10 "$HEALTH_URL/api/items/health" >/dev/null
                '''
            }
        }
    }
