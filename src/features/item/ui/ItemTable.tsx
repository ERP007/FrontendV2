...
export function ItemTable({ header, items, onToggleActive }: ItemTableProps) {
  const columns = useMemo<ColumnDef<Item>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-bold text-ink">{formatNumber(row.original.defaultSafetyStock)}</span>
        ),
        header: '안전재고 기본',
        meta: { align: 'right' },
        size: 110,
  ...
  return <FgDataTable columns={columns} data={items} header={header} />
}
