...
import type { CreateSalesOrderRequest, SalesOrderStatusChangedResponse } from '../model/types'
...
export function useCreateSalesOrderMutation() {
  ...
  return useMutation({
    mutationFn: async (payload: CreateSalesOrderRequest) => {
      const response = await api.post<SalesOrderStatusChangedResponse>('/sales-orders', payload)
